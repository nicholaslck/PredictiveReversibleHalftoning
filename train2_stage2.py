...
def train(config, resume_checkpoint=None):
    ...
    train_set = dataset.HalftoneVOC2012Training(root_dir=dataset_root_dir)
    ...
    loss_func = {
        ...
        "feature_loss": loss.l2_loss,
        "perceptual_loss": loss.l1_loss
    }
...
def train_per_epoch(epoch: tuple, config: dict, model: nn.Module, optimizer: optim.Optimizer, train_loader: DataLoader,
    ...
    for batch_idx, (color_input, gray_input, cvt_input, halftone_input) in enumerate(train_loader):
        ...
        # feature_loss = loss_func['feature_loss'](pseudo_y_pred, gray_input)
        # vgg_loss = loss_func['vgg_loss'](convert.gray2bgr(y_pred / 2. + 0.5), convert.gray2bgr(gray_input / 2. + 0.5))
        # perceptual_loss = loss_func['perceptual_loss'](y_pred, gray_input)
        content_loss = loss_func['restore_loss'](pseudo_y_pred, gray_input)
        vgg_loss = loss_func['vgg_loss'](convert.gray2bgr(y_pred / 2. + 0.5), convert.gray2bgr(gray_input / 2. + 0.5))
        perceptual_loss = loss_func['perceptual_loss'](y_pred, gray_input)
        f_loss = loss_weights['vgg_loss_weight'] * vgg_loss + perceptual_loss

        loss = content_loss + loss_weights['perceptual_loss_weight'] * f_loss
        ...
        # sp_output[5]: y pred of halftone
        ...
        # _sp_feature_loss = loss_func['feature_loss'](sp_output[7], sp_input[1].to(device))

        # loss = loss_weights["restore_loss_weight"] * (restore_loss + 0 * _sp_restoreLoss) \
        #     + loss_weights["feature_loss_weight"] * (feature_loss + 0 * _sp_feature_loss) \
        #     + loss_weights["vgg_loss_weight"] * vgg_loss \
        #     + loss_weights["perceptual_loss_weight"] * perceptual_loss
        ##############################
        # loss = loss_weights["restore_loss_weight"] * restore_loss \
...
def test_per_epoch(epoch: tuple, config: dict, model: nn.Module, test_data_loader: DataLoader, loss_func: dict):
    ...
    with torch.inference_mode():
        for batch_idx, (color_input, gray_input, cvt_input, halftone_input) in enumerate(test_data_loader):
            ...
            _y_pred = convert.denormalize_tensor(y_pred)
            ...
            if batch_idx == 0:
                ...
                board.log_images(epoch[0], 'grayscale_output', convert.tensor_0_1(_y_pred))
                board.log_images(epoch[0], 'grayscale_input', convert.tensor_0_1(_gray_input))
    metrics = [
        # psnr_restore
        all_metrics[0].mean(),
        all_metrics[0].std(),
...
if __name__ == '__main__':
    ...
    try:
        init(config)
        log("Program train2_stage2 start.")
